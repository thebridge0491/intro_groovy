...
(defn save-template
  ...
  (let [save-dir (format "%s/Templates/lein_new/org.sandbox.groovymixed" to-dir)]
...
(defproject org.sandbox.groovymixed/lein-template (get sys-props "templateVersion" "0.1.0")
  :description ~(get sys-props "description" "A Leiningen template for mixed Groovy project (choices skeleton: skeleton_groovy).")
  :url ~(format "http://%s/%s/groovymixed" (get sys-props "repoHost" "bitbucket.org")
...
;; LEIN_JVM_OPTS -- example: LEIN_JVM_OPTS="-Dskeleton=skeleton_groovy -Dexecutable=yes"
