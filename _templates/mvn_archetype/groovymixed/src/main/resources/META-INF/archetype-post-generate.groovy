...
testFrwk = binding.hasVariable('testFrwk') ? testFrwk : 'spock'
...
if (Files.exists(Paths.get("${dirX}/build/choices/build_tool")) && (buildTool in ['gradle', 'maven', 'ant', 'rake', 'make'])) {
...
if (Files.exists(Paths.get("${dirX}/build/choices/testfrwk")) && (testFrwk in ['spock', 'junit', 'testng'])) {
	def files = glob_files("${dirX}/build/choices/testfrwk/${testFrwk}")
	run_cmd("cp -vR ${files.join(' ')} src/test/groovy/${packageInPathFormat}/",
		dirX)
} else { // default: spock
...
if (Files.exists(Paths.get("${dirX}/src/main/java/${packageInPathFormat}")) && ('yes' != executable)) {
    run_cmd("rm -vf src/main/groovy/${packageInPathFormat}/Main.groovy src/main/java/${packageInPathFormat}/Main.java",
		dirX)
}
...
if (Files.exists(Paths.get("${dirX}/build/choices/ffi_lib")) &&(ffiLib in ['jna', 'swig'])) {
	...
	run_cmd("cp -vR ${files.join(' ')} src/main/groovy/${packageInPathFormat}/",
		dirX)
}
/*
